    ...
    if (tt == nt)
        for i = 1:(ndivx * ndivy)
            ...
            if (abs(coord(i, 2)-(dx / 2.0)) <= 1.0e-8)
                horiCnt = horiCnt + 1;
                horizontal_disps(horiCnt, 1:6) = [coord(i, 1), coord(i, 2), disp(i, 1), disp(i, 2), alpha * dtemp * coord(i, 1), alpha * dtemp * coord(i, 2)];
            end
            if (abs(coord(i, 1)-(dx / 2.0)) <= 1.0e-8)
                vertiCnt = vertiCnt + 1;
                vertical_disps(vertiCnt, 1:6) = [coord(i, 1), coord(i, 2), disp(i, 1), disp(i, 2), alpha * dtemp * coord(i, 1), alpha * dtemp * coord(i, 2)];
            end
        end
    end
