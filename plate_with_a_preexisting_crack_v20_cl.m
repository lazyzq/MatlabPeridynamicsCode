...
coord_disp_pd_750_pwc = zeros(totint, 5);
% Peridynamic displacement and Analytical displacement of all points at time step of nt
coord_disp_pd_1000_pwc = zeros(totint, 5);
coord_disp_pd_1250_pwc = zeros(totint, 5);
...
for tt = 1:nt
    ...
    if (tt == 750)
        for i = 1:totint
           coord_disp_pd_750_pwc(i, 1:5) = [coord(i, 1), coord(i, 2), disp(i, 1), disp(i, 2), dmg(i,1)];
        end
    elseif (tt == 1000)
        for i = 1:totint
           coord_disp_pd_1000_pwc(i, 1:5) = [coord(i, 1), coord(i, 2), disp(i, 1), disp(i, 2), dmg(i,1)];
        end
    elseif (tt == 1250)
        for i = 1:totint
           coord_disp_pd_1250_pwc(i, 1:5) = [coord(i, 1), coord(i, 2), disp(i, 1), disp(i, 2), dmg(i,1)];
        end
    end

    crack_length(tt,1:2) = [ctime, newcrlength];
end

%% plot
colormap jet;
subplot(221)
plot(crack_length(:,1), crack_length(:,2))
scale = 1;
subplot(222)
scatter(coord_disp_pd_750_pwc(:, 1)+scale*coord_disp_pd_750_pwc(:, 3), coord_disp_pd_750_pwc(:, 2)+scale*coord_disp_pd_750_pwc(:, 4), [], coord_disp_pd_750_pwc(:, 5), "filled")
subplot(223)
scatter(coord_disp_pd_1000_pwc(:, 1)+scale*coord_disp_pd_1000_pwc(:, 3), coord_disp_pd_1000_pwc(:, 2)+scale*coord_disp_pd_1000_pwc(:, 4), [], coord_disp_pd_1000_pwc(:, 5), "filled")
subplot(224)
scatter(coord_disp_pd_1250_pwc(:, 1)+scale*coord_disp_pd_1250_pwc(:, 3), coord_disp_pd_1250_pwc(:, 2)+scale*coord_disp_pd_1250_pwc(:, 4), [], coord_disp_pd_1250_pwc(:, 5), "filled")
